...
plot(density(facebook$`Lifetime People who have liked your Page and engaged with your post`))
...
plot(density(facebook$lt_ppl_lkd_egnged))

facebook <- na.omit(facebook)
facebook <- facebook[is.finite(facebook$`Lifetime Post Total Reach`) & is.finite(facebook$log_like) & is.finite(facebook$`Page total likes`), ]

lm_model <- lm(lt_ppl_lkd_egnged ~ `Type` + `Post Month` + log_like + `Page total likes` + I(log_like^2), data = facebook)
